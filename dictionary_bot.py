...
embed_master_list = []
Emoji = namedtuple(
    "Emoji", ["l_arrow", "r_arrow", "sound", "d_arrow", "u_arrow", "books"]
)
...
def command_count(
    ...
    data = open_datajson(os.path.join(filepath, "cogs", "data.json"), guild_id)
...
num_to_tone = {"1": "55", "2": "33", "3": "22", "4": "32", "5": "21"}
...
async def not_found(messageable: discord.abc.Messageable):
    embed = Embed(title="Sorry, I've searched far and wide and couldn't find anything")
...
async def on_ready():
    ...
    with codecs.open(
        os.path.join(filepath, "stephen-li.json"), "r", encoding="utf-8"
    ) as f:
...
def embed_from_rxn(reaction):
    return next(
        iter([x for x in embed_master_list if reaction.message.id == x.message_id]),
        None,
    )
...
async def reaction_handling(reaction, user):
    ...
    if reaction.emoji == emoji_list.l_arrow:
        ...
        audio_link = embed.curr_page().audio_link[embed.curr_page().curr_selection]
        dir_and_name = re.split(
            r"audio/(?=[^/]+$)",
            audio_link.replace("http://www.stephen-li.com/TaishaneseVocabulary/", ""),
            maxsplit=1,
            flags=re.IGNORECASE,
        )
        folder_path = Path(os.path.join(filepath, "tones_audio", dir_and_name[0]))
        audio_path = os.path.join(folder_path, dir_and_name[1])
        folder_path.mkdir(parents=True, exist_ok=True)
        orig = os.path.join(filepath, "orig_audio", "".join(dir_and_name))
        Path(os.path.join(filepath, "orig_audio", dir_and_name[0])).mkdir(
        ...
        if not os.path.isfile(audio_path):
            ...
            with open(orig, "wb") as f:
                f.write(mp3.content)
            pynormalize.process_files([orig], target_dbfs=-12, directory=folder_path)

        if isinstance(reaction.message.channel, discord.channel.DMChannel):
            page = embed.curr_page()
            file = discord.File(
                fp=audio_path,
                filename=page.content.fields[page.curr_selection].name[1:].split("-")[0]
                + ".mp3",
            )
            await reaction.message.channel.send(file=file)
        elif user.voice:
            channel = user.voice.channel
            command_count("audio", reaction.message.guild.id, channel)
            ...
            await reaction.message.channel.send(
                content=f"{user.mention} please connect to a voice channel to listen to audio!",
                delete_after=30,
            )
        ...
        if page.is_details:
            ...
            embed_meanings = Embed(
                title=title,
                description=f"```{chr(10).join(alts)}```",
    ...
    if direction != 0:
        ...
        embed.curr_page().content.set_field_at(
            index=field_index, value=value, name=curr_field.name[1:], inline=False
        )
        ...
        value = value[:3] + "asciidoc\n= " + value[3:index] + " =" + value[index:]
        embed.curr_page().content.set_field_at(
            index=field_index, value=value, name=name, inline=False
        )
        await reaction.message.edit(embed=embed.curr_page().content)
...
async def gc(ctx, *, args):
    ...
    if type(args) == str and not re.search(
        r"[\u4e00-\u9fff\u3400-\u4DBF\u4E00-\u9FCC]", search
    ...
    def sort(series):
        ...
        for index, defn in series.items():
            precedence = next(
                iter([i for i, s in enumerate(defn.split(";")) if search in s]), 0
            )
            try:
                syn_list = 5 - len(
                    [
                        ...
                        if re.search(rf"\b{re.escape(syn)}\b", defn)
                    ]
                ) / max(defn.count(" "), 1)
            except TypeError:
                syn_list = 5
            distance = next(
                iter(
                    [
                        i
                        for i, s in enumerate(re.split(r"[,;.]\s|\s", defn))
                        if search in s
                    ]
                ),
                0,
            ...
            for index, row in search_result.iterrows():
                ...
                try:
                    ...
                    for pin_index, ping in enumerate(pin_num):
                        pin_num[pin_index] = "-".join(
                            [
                                re.sub(
                                    r"([1-9]+)",
                                    lambda x: num_to_tone.get(x.group(), x.group()),
                                    pin,
                                )
                                for pin in ping.split("-")
                            ]
                        )
                ...
                embed.add_field(
                    name=embed_name, value=remove_format(value), inline=False
                )
                add_credit(embed, embed_list, 1)
                embed_list.add_page(embed=embed, audio=[], link=[], selection=0)
    ...
    for index, page in enumerate(embed_list.pages):
        if isinstance(page.content, Embed):
            page.content.set_footer(text=f"Page {index+1} of {embed_list.list_len}")
...
async def sl(ctx, *, args):
    ...
    def sort_sl(word):
        index = re.search(rf"\b{re.escape(search)}\b", word["english"], re.IGNORECASE)
        if index is None:
            index = (200, 200)
        else:
            index = index.span()
    ...
    if re.search("[\u4e00-\u9fff]", search):
        ...
        search_result = [word for word in stephen_li if search in word["taishanese"]]
    else:
        search_result = [
            word
            for word in stephen_li
            if re.search(rf"\b{re.escape(search)}\b", word["english"].lower())
        ]
    search_result = sorted(search_result, key=sort_sl)

    if len(search_result) == 0:
        embed = Embed(title="Sorry, I searched far and wide and couldn't find anything")
        file = discord.File("cat_404.jpg")
        embed.set_image(url="attachment://cat_404.jpg")
        await ctx.send(embed=embed, file=file)
        return
    ...
    for word in search_result:
        ...
        if counter == 0:
            embed_list.add_page(embed=None, audio=[], link=[], selection=0)
            embed = Embed(
                title=f'Words matching "{search}"', colour=Color.from_rgb(163, 255, 163)
            )
            embed_name = "🠶 "
            value = "```asciidoc\n= "
...
async def randomgc(ctx):
    ...
    title = (
        re.sub(
            r"([1-9]+)", lambda x: num_to_tone.get(x.group(), x.group()), row["p.y."]
        )
        + "-"
        + row["繁"]
    )
    ...
    value = f"{row['英译与词句']}"
    embed = Embed(
        title=title, description=f"```{value}```", colour=Color.from_rgb(186, 143, 255)
    )
    await ctx.send(embed=embed)
...
async def leave_channel(ctx):
    ...
    await ctx.send(
        content=f"{ctx.author.mention} please connect to a voice channel to use the command!",
        delete_after=30,
    )
...
async def hoisan_sauce(ctx):
    ...
    if ctx.author.voice:
        ...
        file = discord.File(
            os.path.join(mypath, "gifs", gifs[chosen_file.replace(".mp3", "")]),
        ...
        await ctx.send(
            content=f"{ctx.author.mention} please connect to a voice channel to listen to audio!",
            delete_after=30,
        )
...
def num_to_accent(input_string):
    ...
    items = [
        phrase.groups()
        for phrase in re.finditer(r"([a-z]+)([0-9]+)\b", input_string, re.I)
    ]
    for word_num in items:
        positions = [
            index + 1 for index, char in enumerate(word_num[0]) if char in vowels
        ]
        if len(positions) > 1:
            return_list.append(
...
async def penyim(ctx, *, args):
    ...
    if type(args) == str and not re.search("[\u4e00-\u9fff]", args):
        args = [args]
    search_result = []
    search_pinyin = []
    for word in args:
        if re.search("[\u4e00-\u9fff]", word):
            ...
            if row.empty:
                ...
                row = row.sort_index(
                    key=lambda index: pd.Series(
                        [len(hed_usage(num)) for num in index.tolist()]
                    ),
                    ascending=False,
                )
            row = row.iloc[0]
    ...
    conversion = (
        f'{"".join(args)} -> {" ".join(search_result)} ({" ".join(search_pinyin)})'
    )
    if len(conversion) < 50:
        embed = Embed(title=conversion, colour=Color.from_rgb(181, 255, 158))
        ...
        embed = Embed(description=conversion, colour=Color.from_rgb(181, 255, 158))
    else:
...
async def leave_server(ctx, guild_id=None):
    ...
    def check(response):
        return (
            response.author.id == 693267245610303518
            and response.content == guild_toleave.name
        )
    ...
    if not message.guild:
        if len(message.embeds) > 0:
            await command_channel.send(
                embed=message.embeds[0], content=f"{message.author} sent an embed"
            )
        ...
        if message.content != "":
            await command_channel.send(
                f"{message.author} ({message.author.id}) said {message.content}"
            )
